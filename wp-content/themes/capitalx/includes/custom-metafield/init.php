...
class cmb_Meta_Box {
	...
	public function register_scripts() {
			...
			if ( ! is_admin() ) {
		   	...
		   	wp_register_script( 'wp-color-picker', admin_url( 'js/color-picker.min.js' ), array( 'iris' ), self::CMB_VERSION );
				wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
					'clear'         => __( 'Clear', 'capitalx'),
					'defaultString' => __( 'Default', 'capitalx'),
					'pick'          => __( 'Select Color', 'capitalx'),
					'current'       => __( 'Current Color', 'capitalx'),
				) );
			}
		} else {
		...
		wp_register_script( 'cmb-fontpicker', CMB_META_BOX_URL . 'js/fontawesome-iconpicker.js' );
		wp_register_script( 'cmb-scripts', CMB_META_BOX_URL .'js/cmb'. $min .'.js', $scripts, self::CMB_VERSION );
		wp_register_script( 'cmb-fontmin', CMB_META_BOX_URL .'js/cmb'. $min .'.js', $scripts, self::CMB_VERSION );
		...
		wp_register_style( 'cmb-font', CMB_META_BOX_URL . 'font-awesome.min.css', $styles );
		wp_register_style( 'cmb-fontawesome', CMB_META_BOX_URL . 'fontawesome-iconpicker.min.css', $styles );
		wp_register_style( 'cmb-linefont', CMB_META_BOX_URL . 'simple-line-icons.css', $styles );
	...
	public function do_scripts( $hook ) {
		...
		if ( $hook == 'post.php' || $hook == 'post-new.php' || $hook == 'page-new.php' || $hook == 'page.php' ) {
			...
			wp_enqueue_script( 'cmb-fontpicker' );
			// default is to show cmb styles on post pages
			if ( $this->_meta_box['cmb_styles'] )
				wp_enqueue_style( 'cmb-styles' );
				wp_enqueue_style( 'cmb-font' );
				wp_enqueue_style( 'cmb-fontawesome' );
				wp_enqueue_style( 'cmb-linefont' );
		}
	}
	/**
		...
		<script>
		 jQuery(document).ready(function(){
            jQuery(function() {
                ...
                jQuery(document).on('click', '.action-placement', function(e) {
                    jQuery('.action-placement').removeClass('active');
                    jQuery(this).addClass('active');
                    jQuery('.icp-opts').data('iconpicker').updatePlacement($(this).text());
                    e.preventDefault();
                    return false;
                });
                jQuery('.action-create').on('click', function() {
                    jQuery('.icp-auto').iconpicker();
                    jQuery('.icp-dd').each(function() {
                        var $this = $(this);
                        $this.iconpicker({
                ...
                // by user interaction
                jQuery('.icp').on('iconpickerSelected', function(e) {
                    jQuery('.lead .picker-target').get(0).className = 'picker-target fa-3x ' +
                            e.iconpickerInstance.options.iconBaseClass + ' ' +
                            e.iconpickerInstance.getValue(e.iconpickerValue);
                });
            });
			
			jQuery('.fontline').click(function() {
				jQuery('.iconpicker-popovernew').show();
			});
			jQuery(document).click(function(e) {
		        if (!jQuery(e.target).is('.iconpicker-popovernew, .fontline, .iconpicker-popovernew *')) {
		            jQuery(".iconpicker-popovernew").hide();
		        }
		    });
		    jQuery('.iconpicker-popovernew .glyph-item').click(function() {
		    	var linefontval = jQuery(this).attr("data-js-prompt");
		    	jQuery('.fontline').val(linefontval);
		    });
		});
        </script>
	<?php }
...
function cmb_metabox_form( $meta_box, $object_id, $echo = true ) {
	...
	$form = sprintf( $form_format, $meta_box['id'], $object_id, $form, __( 'Save', 'capitalx') );
	if ( $echo )
		echo $form;
	return $form;
}
