<?php
/*
Plugin Name: Capitalx Short Code builder
Plugin URI: 
Description:  Capitalx Short Code  Plugin for Wordpress Themes
Version: 1.0.0
Author: Vanbreadshaw
Author URI: 
*/
...
define ('VEE_PB_BUILDER_FA_PATH' , plugin_dir_path( __FILE__ ) . 'tinymce/css/font-awesome.css' );
if( ! class_exists( 'Vee_Pb_Plugin' ) ) {
	class Vee_Pb_Plugin {
		/**
		...
		private function __construct() {
			define('VEE_PB_TINYMCE_URI', plugin_dir_url( __FILE__ ) . 'tinymce');
			define('VEE_PB_TINYMCE_DIR', plugin_dir_path( __FILE__ ) .'tinymce');
			add_action('init', array(&$this, 'init'));
			add_action('admin_init', array(&$this, 'admin_init'));
			add_action('after_setup_theme', array(&$this, 'load_ve_pb_text_domain'));
			//add_action('admin_init', array(&$this, 'updater'));
			add_action('wp_ajax_shortcodes_popup', array(&$this, 'popup'));
		}
		/**
		 ...
		 * @return void
		 */		
		function load_ve_pb_text_domain() {
			load_plugin_textdomain( 'vee-core', false, dirname( plugin_basename(__FILE__) ) . '/languages' );
		}
		// --------------------------------------------------------------------------
		/**
		...
		public static function attributes( $slug, $attributes = array() ) {
			$out = '';
			$attr = apply_filters( "ve_pb_attr_{$slug}", $attributes );
			if ( empty( $attr ) ) {
				$attr['class'] = $slug;
			}
		...
		public static function vee_link_pages() {		
			wp_link_pages( array(
				'before'      => '<div class="page-links"><span class="page-links-title">' . __( 'Pages:', 'vee-core' ) . '</span>',
				'after'       => '</div>',
		...
		{
			if( is_admin() ) {
				$plugin_array['short_button'] = VEE_PB_TINYMCE_URI . '/plugin.js';
			}
			return $plugin_array;
		}
		// --------------------------------------------------------------------------
		...
		{
			array_push( $buttons, 'short_button' );
			return $buttons;
		}
		/**
		...
		{
			// css
			wp_enqueue_style( 've_pb-popup', VEE_PB_TINYMCE_URI . '/css/popup.css', false, Vee_Pb_Plugin::VERSION, 'all' );
			wp_enqueue_style( 've_pb-jquery.chosen', VEE_PB_TINYMCE_URI . '/css/chosen.css', false, Vee_Pb_Plugin::VERSION, 'all' );
			wp_enqueue_style( 've_pbn-font-awesome', VEE_PB_TINYMCE_URI . '/css/font-awesome.css', false, Vee_Pb_Plugin::VERSION, 'all' );
			wp_enqueue_style( 've_pbn-jquery-te', VEE_PB_TINYMCE_URI . '/css/jquery-te-1.4.0.css', false, Vee_Pb_Plugin::VERSION, 'all' );
			wp_enqueue_style( 'wp-color-picker' );
			// js
			wp_enqueue_script( 'jquery-ui-sortable' );
			wp_enqueue_script( 've_pb-jquery-livequery', VEE_PB_TINYMCE_URI . '/js/jquery.livequery.js', false, Vee_Pb_Plugin::VERSION, false );
			wp_enqueue_script( 've_pb-jquery-appendo', VEE_PB_TINYMCE_URI . '/js/jquery.appendo.js', false, Vee_Pb_Plugin::VERSION, false );
			wp_enqueue_script( 've_pb-jquery-te', VEE_PB_TINYMCE_URI . '/js/jquery-te-1.4.0.min.js', false, Vee_Pb_Plugin::VERSION, false );
			wp_enqueue_script( 've_pb-jquery.chosen', VEE_PB_TINYMCE_URI . '/js/chosen.jquery.min.js', false, Vee_Pb_Plugin::VERSION, false );
			wp_enqueue_script( 'wp-color-picker' );
			wp_enqueue_script( 've_pb-popup', VEE_PB_TINYMCE_URI . '/js/popup.js', false, Vee_Pb_Plugin::VERSION, true );
			// Developer mode
			$dev_mode = current_theme_supports( 'shortcodes_embed' );
			if( $dev_mode ) {
				$dev_mode = 'true';
			} else {
				$dev_mode = 'false';
			}
			wp_localize_script( 've_pb-popup', 'Ve_pagebuilderShortcodes', array('plugin_folder' => plugins_url( '', __FILE__ ), 'dev' => $dev_mode) );
		}
		/**
		 ...
		 */
		function popup() {
			require_once( VEE_PB_TINYMCE_DIR . '/ve_pagebuilder-sc.php' );
			die();
		}
...
add_action( 'plugins_loaded', array( 'Vee_Pb_Plugin', 'get_instance' ) );
// Blocking Script
if( ! function_exists( 'vee_pb_block_direct_access' ) ) {
	/**
	 * Blocks direct accessing of a core file
	 * @param  none
	 * @return void
	 */
	function vee_pb_block_direct_access() {
		if( ! defined( 'ABSPATH' ) ) {
...
* Remove extra P tags
*----------------------------------------------------------------------------*/
function vee_pb_shortcodes_formatter($content) {
	$block = join("|",array("section","section_header", "pricing_table","price_column", "buynow", "services", "counter", "button","fun_facts",  
	"funfacts","aboutus", "about_val",  "ourwork", "work",   "map",  "blog", ));
	// opening tag
...
add_filter('the_content', 'vee_pb_shortcodes_formatter');
add_filter('widget_text', 'vee_pb_shortcodes_formatter');
// Creating of custom post types
    function capitalx_add_cpt() {
		
	 
		// Services
		 $labels = array(
		    'name' => __('Services', 'vee-core'),
		    'singular_name' => __('Services', 'vee-core'),
		    'add_new' => __('New Services', 'vee-core'),
		    'add_new_item' => __('Add New Services', 'vee-core'),
		    'edit_item' => __('Edit Services', 'vee-core'),
		    'new_item' => __('All Services', 'vee-core'),
		    'view_item' => __('View Services', 'vee-core'),
		    'search_items' => __('Search Services', 'vee-core'),
		    'not_found' =>  __('Services not found', 'vee-core'),
		    'not_found_in_trash' => __('Services', 'vee-core'),
		    'parent_item_colon' => '',
		    'menu_name' => __('Services', 'vee-core')
	    );
	    $args = array(
		    'labels' => $labels,
		    'public' => true,
		...
		// OUR TEAM
		 ...
		 $labels = array(
		    'name' => __('Manage Members', 'vee-core'),
		    'singular_name' => __('Manage Members', 'vee-core'),
		    'add_new' => __('Add New Member', 'vee-core'),
		    'add_new_item' => __('Add New Member', 'vee-core'),
		    'edit_item' => __('Edit Team Member', 'vee-core'),
		    'new_item' => __('All Team Members', 'vee-core'),
		    'view_item' => __('View Team Member', 'vee-core'),
		    'search_items' => __('Search Team Member', 'vee-core'),
		    'not_found' =>  __('Team Member not found', 'vee-core'),
		    'not_found_in_trash' => __('Team Member not found in cart', 'vee-core'),
		    'parent_item_colon' => '',
		    'menu_name' => __('Team Member', 'vee-core')
	    );
	    $args = array(
		    'labels' => $labels,
		    'public' => true,
	...
	// Our Testimonials
		 $labels = array(
		    'name' => __('Manage Reviews', 'vee-core'),
		    'singular_name' => __('Review', 'vee-core'),
		    'add_new' => __('Add New Review', 'vee-core'),
		    'add_new_item' => __('Add New Review', 'vee-core'),
		    'edit_item' => __('Edit Review', 'vee-core'),
		    'new_item' => __('All Reviews', 'vee-core'),
		    'view_item' => __('View Review', 'vee-core'),
		    'search_items' => __('Search Reviews', 'vee-core'),
		    'not_found' =>  __('Reviews not found', 'vee-core'),
		    'not_found_in_trash' => __('Reviews', 'vee-core'),
		    'parent_item_colon' => '',
		    'menu_name' => __('Reviews', 'vee-core')
	    );
	    $args = array(
		    'labels' => $labels,
		    'public' => true,
		...
		// Our Client
		 $labels = array(
		    'name' => __('Client', 'vee-core'),
		    'singular_name' => __('Client', 'vee-core'),
		    'add_new' => __('New Client', 'vee-core'),
		    'add_new_item' => __('Add New Client', 'vee-core'),
		    'edit_item' => __('Edit Client', 'vee-core'),
		    'new_item' => __('All Client', 'vee-core'),
		    'view_item' => __('View Client', 'vee-core'),
		    'search_items' => __('Search Client', 'vee-core'),
		    'not_found' =>  __('Client not found', 'vee-core'),
		    'not_found_in_trash' => __('Client', 'vee-core'),
		    'parent_item_colon' => '',
		    'menu_name' => __('Clients', 'vee-core')
	    );
	    $args = array(
		    'labels' => $labels,
		    'public' => true,
    ...
    add_action('init', 'capitalx_add_cpt', 100);
